...
module.exports = function errorHandler(err, req, res, next) { // eslint-disable-line no-unused-vars
    ...
    if (redirectPage.file) {
      send(req, redirectPage.file, {index: false, root: routerConfig.workingDir})
        .on('error', function (err) {
          ...
          endRequest(res, status, headers, verr, routerConfig);
        })
  ...
  endRequest(res, status, headers, err, routerConfig);
};

function endRequest(res, status, headers, err, routerConfig) {
  if (res.headersSent) {
    return;
  }

  if (routerConfig && routerConfig.returnErrorSource) {
    res.setHeader('x-error-source', 'gateway');
  }
