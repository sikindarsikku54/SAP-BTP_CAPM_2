...
module.exports = function(req, matchHttpMethods /* default undefined, does not make sense for WebSockets */) {
  ...
  // Get and remove sap_idp param from url
  let parsedQuery = urlUtils.getAllQueryParams({url});
  req.dynamicIDP = parsedQuery && parsedQuery[urlUtils.SAP_IDP_QUERY_PARAM];
  if (req.dynamicIDP) {
    url = urlUtils.removeQueryParamFromUrl(url, urlUtils.SAP_IDP_QUERY_PARAM);
  }
  ...
  for (let i = 0; i < routes.length; i++) {
    ...
    if (!matchHttpMethods || httpMethodsMatch(route.httpMethods, req.method)) {
      return applyRewriteRule(req, route, routerConfig, url);
    }
...
function applyRewriteRule(req, route, routerConfig, url) {
  ...
  let destinationsHtml5RepoEndpoints = destination && req.destinationCredentials
      && req.destinationCredentials['html5-apps-repo'] && req.destinationCredentials['html5-apps-repo']['endpoints'];
  if (destinationsHtml5RepoEndpoints) {
    try {
      destinationsHtml5RepoEndpoints = JSON.parse(destinationsHtml5RepoEndpoints);
    } catch (err) {
      throw new Error('Failed to parse destinations from html5 repo destination ' + err);
    }
  }
  ...
  if (!url) {
    url = dynamicRoutingUtils.getCoreUrl(req);
  }
  ...
  if (destination) {
    ...
    const oDestination = (routerConfig.destinations && routerConfig.destinations[destination])
        || (destinationsHtml5RepoEndpoints && destinationsHtml5RepoEndpoints[destination])
