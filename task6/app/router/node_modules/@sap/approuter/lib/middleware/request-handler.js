...
function removeNoCacheSetCookieIfNeeded(incomingResponse) {
  if (!incomingResponse.getHeader('set-cookie')) {
    let ccHeader = incomingResponse.getHeader('cache-control');
    if (ccHeader) {
      let ccValues = Array.isArray(ccHeader) ? ccHeader : [ccHeader];
      let newValues = [];

      ccValues.forEach(val => {
        let directives = val.split(',').map(d => d.trim().toLowerCase());
        directives = directives.filter(d => d !== 'no-cache="set-cookie"');

        if (directives.length > 0) {
          newValues.push(directives.join(', '));
        }
      });
      if (newValues.length === 0) {
        incomingResponse.removeHeader('cache-control');
      } else {
        incomingResponse.setHeader(
          'cache-control',
...
function setCookiesFromBackend(req, res, backendCookies, tracer) {
  const isService2Approuter = req.headers && req.headers['x-approuter-authorization'];
  const encryptedBackendCookie = res.getHeader('set-cookie');
  ...
  if (isService2Approuter && encryptedBackendCookie){
    ...
    backendCookies.nonSessionCookies.length && res.setHeader(headerName, backendCookies.nonSessionCookies.concat(encryptedBackendCookie));
  } else {
    // send cookies from the backend to browser only if approuter is configured to do so
    if (setBackendCookiesEnabled(req)) {
      const allCookies = backendCookies.nonSessionCookies.concat(backendCookies.sessionCookies);
      allCookies.length && res.setHeader(headerName, allCookies);
      tracer.debug('sending non session and session backend cookies');
    } else {
...
function onResponse(requests, requestStats, next) {
  ...
  if (!setBackendCookiesEnabled(incomingRequest) && incomingRequest.session && backendCookies.sessionCookies.length > 0) {
  ...
  for (let headerName in outgoingResponse.headers) {
    if (headerName === 'set-cookie') {
      setCookiesFromBackend(incomingRequest, incomingResponse, backendCookies, tracer);
  ...
  removeNoCacheSetCookieIfNeeded(incomingResponse);
  ...
  if (incomingRequest.httpVersionMajor !== 2) {
...
function handleBackendError(incomingRequest, incommingResponse){
  if (incommingResponse.statusCode > 399 && incommingResponse.statusCode < 600){
    if (incommingResponse.statusCode === 401 || incommingResponse.statusCode === 403) {
      if (incomingRequest.app.services
        && incomingRequest.internalUrl.route
        && incomingRequest.internalUrl.route.service
        && incomingRequest.app.services[incomingRequest.internalUrl.route.service]){
        delete incomingRequest.app.services[incomingRequest.internalUrl.route.service].token;
        const tenant = incomingRequest.tenant || passportUtils.getUrlTenant(incomingRequest);
        if (tenant && incomingRequest.app.services[incomingRequest.internalUrl.route.service][tenant]) {
          delete incomingRequest.app.services[incomingRequest.internalUrl.route.service][tenant].token;
        }
      }
    }
...
module.exports.removeNoCacheSetCookieIfNeeded = removeNoCacheSetCookieIfNeeded;
