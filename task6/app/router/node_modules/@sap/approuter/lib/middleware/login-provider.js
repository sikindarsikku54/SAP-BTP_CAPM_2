...
module.exports = {
  isLoginRequired: function (req) {
    if (!req.internalUrl && process.env.REQUIRE_AUTH_FOR_UNKNOWN_ROUTES !== 'true') {
      return false;
    }
  ...
  exchangeToken: async function (req, cb) {
    ...
    if (req.destinationCredentials.ias && req.zoneInfo) {
      ...
      if (req.destinationCredentials.ias.sharedIASToken) {
        ...
        if (req.destinationCredentials.uaa && req.destinationCredentials.uaa.clientid) {
          // Check if any routes require scopes before attempting XSUAA exchange
          // Only apply the optimization if we have a proper appConfig with routes defined
          const scopesRequired = req.routerConfig && req.routerConfig.appConfig && req.routerConfig.appConfig.routes ?
            authorizationHandler.isScopeCheckRequired(req.routerConfig.appConfig) : true;

          if (!scopesRequired) {
            // Skip XSUAA exchange - no scopes needed anywhere
            logRequestInfo(req, 'Skipping XSUAA token exchange - no routes require scopes');
          ...
          // Original XSUAA exchange logic continues here...
...
function updateSession(req, iasExchangedToken, xsuaaExchangedToken) {
  ...
  let xsuaaExchangedTokenDecoded = xsuaaExchangedToken ? jwtDecode(xsuaaExchangedToken) : null;
