...
startCachedCredentialsLogsInterval(tracer);
...
module.exports.instanceDestinationBSCredentialsCache = instanceDestinationBSCredentialsCache;
module.exports.startCachedCredentialsLogsInterval = startCachedCredentialsLogsInterval;
...
function getDestinationCredentialsBySapCloudService(req, sapCloudService, cb) {
  ...
  module.exports.fetchDestinationBSCredentials(req, tracer, function (err, result) {
    if (err) {
      return cb(err);
    }
    let destinationBSCredentials = result.destinationBSCredentials;
...
function cacheBSDestinations(req, cb) {
  let specificStdTTL = getValidatedEnvInt('STDTTL', stdTTL);
  let tracer = req.loggingContext.getTracer(__filename);
  ...
  fetchDestinationBSCredentials(req, tracer, function (err, result) {
    if (err) {
    ...
    let fetchedBSCredentials = result.destinationBSCredentials;
...
function downloadHTML5Application(req, res) {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    let html5RepoToken;
    try {
      html5RepoToken = await html5RepoUtils.getHTML5RepoToken(req);
    } catch (e) {
      return reject(`Failed to download html5 application , ${e.message}`);
    }
    module.exports.getHTML5Applications(req, async function (err, html5Applications) {
      ...
      if (appHostId === null) {
        const error = new Error('Application ' + appKey.entireKey + ' not found');
        error.statusCode = 404;
        return reject(error);
      }
      ...
      let requestOptions = {
        ...
        cache: {services: req.app.services, serviceName: html5RTService},
        service: vcapUtils.HTML5_APPS_REPO
      };

      const logInfo = passportUtils.getTenantInfo(req).logInfo;
      logInfo && (requestOptions.headers['tenantid'] = logInfo);
      await request.pipe(requestOptions, res, reject);
      resolve();
    });
  });
}
        ...
        .then(values => {
          ...
          values.forEach((value) => {
            if (value.applications && value.applications.length > 0) {
              let appArrToMerge = [appResult.applications, value.applications];
              appResult.applications = Array.prototype.concat(...appArrToMerge);
            }
            if (value.error) {
              appResult.errors.push(value.error);
            }
          });
          if (result.errors) {
            appResult.errors = appResult.errors.concat(result.errors);
          }
          subscribedApplicationsUtils.addSubscribedApplications(req, appResult, cb);
        })
        .catch(err => cb(err));
    } else {
      cb(null, {});
...
// Generate errors for duplicate sap.cloud.service values in subaccount/instance destinations
function catchErrorDependingOnLevel(tracker, level, errors) {
  for (const serviceName in tracker) {
    if (tracker[serviceName].length > 1) {
      let errorMessage = `Multiple ${level} level destinations found with same sap.cloud.service ${serviceName}: ` +
        tracker[serviceName].join(', ') +
        '. This creates ambiguity as only the last destination will be used.';
      tracer.error(errorMessage);

      // Create an entry for each destination with duplicate sap.cloud.service
      tracker[serviceName].forEach(function (destinationName) {
        let otherDestinations = tracker[serviceName].filter(name => name !== destinationName);
        errors.push({
          destinationName: destinationName,
          sapCloudService: level === 'instance' ? serviceName.split(' ')[0] : serviceName,
          reason: 'sap.cloud.service ' + serviceName + ' is also configured in destinations: ' +
            otherDestinations.join(', ') + `, multiple ${level} level destinations with same sap.cloud.service property not allowed`
        });
      });
    }
  }
...
function fetchDestinationBSCredentials(req, tracer, cb) {
  ...
  let subaccountServiceTracker = {};
  let instanceServiceTracker = {};
  const errors = [];
  ...
  getDestinationsFromDestinationService(req, function (err, destinationsData) {
    ...
    destinationsData.forEach(function (destination) {
      if (destination.hasOwnProperty('sap.cloud.service')) {
        ...
        if (!destination.hasOwnProperty('app_host_id')) {
          // Track instance level destinations with same sap.cloud.service for duplicate warning
          if (destination.destinationId) {
            const instanceLevelKey = serviceName + ' ' + destination.destinationId;
            if (!instanceServiceTracker[instanceLevelKey]) {
              instanceServiceTracker[instanceLevelKey] = [];
            }
            instanceServiceTracker[instanceLevelKey].push(destination.Name);
          } else {
            // Track subaccount level destinations with same sap.cloud.service for duplicate warning
            if (!destination.hasOwnProperty('html5-apps-repo')) {
              if (!subaccountServiceTracker[serviceName]) {
                subaccountServiceTracker[serviceName] = [];
              }
              subaccountServiceTracker[serviceName].push(destination.Name);
            }
          }
        }
    ...
    catchErrorDependingOnLevel(subaccountServiceTracker, 'subaccount', errors);
    catchErrorDependingOnLevel(instanceServiceTracker, 'instance', errors);
    ...
    let result = { destinationBSCredentials: destinationBSCredentials };
    if (errors.length > 0) {
...
function getApplicationMetadataPromise(req, options) {
  ...
  return new Promise(async (resolve, reject) => {
    let html5Cred = vcapUtils.getServiceCredentials({tag: html5RTService});
    // Check if html5Cred is null
    if (!html5Cred || !html5Cred.uri) {
      return reject(`HTML5 service credentials not found or missing URI for service tag: ${html5RTService}`);
    }
    let html5RepoToken;
    try {
      html5RepoToken = await html5RepoUtils.getHTML5RepoToken(req);
    } catch (e) {
      return reject(`Failed to get html5 application metadata, ${e.message}`);
    }
    ...
    // Add enableRetry flag to request options
    requestOptions.enableRetry = true;

    const logInfo = passportUtils.getTenantInfo(req).logInfo;
    logInfo && (requestOptions.headers['tenantid'] = logInfo);

    request.get(requestOptions, function onResponse(err, res, body) {
      // give out 500 error if client (axios) timeout occurs
      if (err && err.code === 'ECONNABORTED') {
        tracer.error(`${requestOptions.url} with timeout=${requestOptions.timeout}ms -> ${err.message}`);
        reject(err);
      }
      let result = {};
      if (err || res.statusCode !== 200) {
        result.error = {
          ...
          reason: res && res.body ? res.body : 'Failed to get metadata, ' + (err || (res && res.statusCode && `response status code: ${res.statusCode}`)) + '. Please check the service instance exists.'
        };
      } else {
        let responseJSON = JSON.parse(body);
        if (responseJSON.length > 0) {
          responseJSON.forEach(function (app) {
            if (app.applicationType === html5RepoUtils.technicalCacheBusterApplication) {
              return;
            }
...
function mergeHTMLRepoDestinations(destinationBSCredentials, html5RepoDestinations) {
  ...
  for (let sapCloudServiceKey in html5RepoDestinations) {
    if (destinationBSCredentials[sapCloudServiceKey] && destinationBSCredentials[sapCloudServiceKey].credentials) {
      ...
      if (html5RepoDestinations[sapCloudServiceKey].IASDependencyName) {
        destinationBSCredentials[sapCloudServiceKey].credentials['ias'] = {
      ...
      destinationBSCredentials[sapCloudServiceKey] = {
        credentials: {
          ...
          uniqueServiceName: sapCloudServiceKey
        }
      };
    }
  }
...
function getDestinationsFromDestinationService(req, cb) {
  ...
  if (req.destinationKey) {
    destinationUtils.getDestinationsByDestinationKey(req.app, req.destinationKey,null, headerUtils.getCorrelationId(req), true, function (err, instanceDestinations) {
      if (err) {
        return cb(err);
      }
...
function collectXsuaaOrXsuaaAppHostDestinations(destination, destinationBSCredentials, serviceName, destinationXsappname, sapCloudServiceKey, tracer) {
  ...
  for (let prop in destination) {
    ...
    if (prop === 'HTML5.IASDependencyName' || prop === 'IASDependencyName') {
      destinationBSCredentials[sapCloudServiceKey].credentials['ias'] = {
        IASDependencyName: destination[prop]
      };
    }
    ...
    if (prop === 'clientId') {
      ...
      if (!destinationBSCredentials[sapCloudServiceKey].credentials['uaa']['uaadomain'] && ownUAACredentials) {
...
function startCachedCredentialsLogsInterval(tracer) {
  if ((process.env.ENABLE_MEASURE_CACHE_KEYS && process.env.ENABLE_MEASURE_CACHE_KEYS === 'false') || !process.env.SAAS_APPROUTER) {
    return;
  }
  if (process.env.CF_INSTANCE_INDEX === '0') {
    let numOfMinutes = parseInt(process.env.LOG_SIZE_INTERVAL, 10);
    if (isNaN(numOfMinutes) || numOfMinutes <= 0) {
      numOfMinutes = 60;
    }
    let LOG_INTERVAL_MS = numOfMinutes * 60 * 1000;
    setInterval(() => {
      tracer.error('tenantDestinationBSCredentialsCache stats:', tenantDestinationBSCredentialsCache.getStats());
      tracer.error('instanceDestinationBSCredentialsCache stats:', instanceDestinationBSCredentialsCache.getStats());
      tracer.error('embeddedCredentialsCache stats:', html5RepoUtils.html5AppsCache.getStats());
    }, LOG_INTERVAL_MS);
  }
}