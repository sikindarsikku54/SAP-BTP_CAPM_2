...
const sapCookies = ['cmapi_cookie_privacy'];
...
module.exports.setBackendCookiesEnabled = function (req) {
  return req.internalUrl && req.internalUrl.route && req.internalUrl.route.setBackendSessionCookies;
};
...
exports.ivConstantAes256Fallback = Buffer.allocUnsafe(16);
exports.key.copy(exports.ivConstantAes256Fallback);
...
exports.addSessionCookie = function (req, res) {
  ...
  // Generate TWO separate UUIDs for security:
  // - sessionCookieValue: Secret, stored in HttpOnly cookie, never in URL
  // - stateValue: Public, appears in OAuth redirect URL for CSRF protection
  // This separation prevents attackers who can read URLs from impersonating sessions
  const sessionCookieValue = uuid.v4();
  const stateValue = uuid.v4();

  req.state = stateValue;                    // Public: will appear in URL
  req.preAuthSessionId = sessionCookieValue; // Secret: only in cookie

  // Store mapping from state to cookie for validation on callback
  state.setStateToCookieMapping(stateValue, sessionCookieValue);

  // Store cookie value in state cache so isActiveStateCookie can identify
  // this cookie as part of an active OAuth flow (O(1) lookup)
  state.setCacheStateValue(sessionCookieValue);
...
// Encryption (secure mode: aes-256-gcm)
exports.encryptCookies = function (cookies, backendName) {
  const iv = crypto.randomBytes(12); // 12 bytes for GCM
  const cipher = crypto.createCipheriv('aes-256-gcm', exports.key, iv);
  const data = JSON.stringify({[backendName]: cookies.map(cookie => cookie.key + '=' + cookie.value)});
  const msg = [cipher.update(data, 'utf8', 'hex'), cipher.final('hex')];
  const tag = cipher.getAuthTag().toString('hex');
  // Store IV, tag, and ciphertext
  const result = `${iv.toString('hex')}:${tag}:${msg.join('')}`;
  return exports.getBackendCookieName() + '=' + result + '; Path=/; HttpOnly;';
};

function aes256Fallback(encryptedCookie) {
  const decipher = crypto.createDecipheriv('aes256', exports.key, exports.ivConstantAes256Fallback);
  const msg = [];
...
exports.decryptCookies = function (cookies, backendName, logger) {
  ...
  if (encryptedCookie) {
    ...
    try {
      ...
      const [ivHex, tagHex, encryptedHex] = encryptedCookie.value.split(':');
      if (tagHex === undefined) {
        // fallback to aes256 without gcm
        logger && logger.info('Cookies %s use old format - fallback', cookies);
        msg = aes256Fallback(encryptedCookie);
      } else {
        const iv = Buffer.from(ivHex, 'hex');
        const tag = Buffer.from(tagHex, 'hex');
        const decipher = crypto.createDecipheriv('aes-256-gcm', exports.key, iv);
        decipher.setAuthTag(tag);
        msg.push(decipher.update(encryptedHex, 'hex', 'utf8'));
...
exports.serialize = function (req, name, val, options) {
  ...
  try {
    try {
      ...
      if ((req && req.routerConfig && req.routerConfig.cookieBackwardCompatibility) || sapCookies.includes(name)) {
