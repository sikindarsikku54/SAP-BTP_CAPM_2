...
module.exports.writeToAuditLog = function (req, loggingData, message, cb) {
  ...
  req.app.auditLogger && req.app.auditLogger.securityMessage(message)
    .by(userObject.user)
...
module.exports.getAuditLogAdditionalData = function (req)  {
  let tenantid = req.session.user && req.session.user.tenantid;
  let tenant = req.session.user && req.session.user.tenant;
  ...
  return {user: userToLog, IP: IP, tenantid: tenantid, tenant: tenant};
};
...
function checkPlan(req, loggingData) {
  ...
  if (req.app && req.app.auditLogger && req.app.auditLogger.auditlogCredentials && req.app.auditLogger.auditlogCredentials.uaa) {
    userObject.user = '$USER';

    let tenantInfo = passportUtils.getTenantInfo(req);
    userObject.tenantid = !tenantInfo.multitenant ? '$PROVIDER' : '$SUBSCRIBER';
    userObject.subdomain = !tenantInfo.multitenant ? null : (loggingData.tenant || tenantInfo.tenant);
