...
let tokenUtils          = require('./token-utils');
let headerUtils         = require('./header-util');
const html5RepoService    = 'html5-apps-repo-rt';
const url                 = require('url');
const {getValidatedEnvInt} = require('./env-utils');
const stdTTL              = getValidatedEnvInt('HTML5_APPS_CACHE', 300);
...
let passportUtils = require('../passport/utils');
...
const destinationNormalization = require('./destination-normalization');
let bsUtils;


const HttpsAgentKeepAlive = require('agentkeepalive').HttpsAgent;
const agentTimeOut = getValidatedEnvInt('HTML5_REPO_KEEP_ALIVE_TIMEOUT', 60000);
...
module.exports.handleDestinations                = handleDestinations;
...
async function enrichWithHTML5RepoData(cachedApps,queryParams,req) {
  ...
  const errors = (cachedApps && cachedApps.errors) || [];
  const result = {
    applications: [],
    errors
  };
  ...
  // Create a Set to track processed appHostIds to avoid duplicates
  const processedAppHostIds = new Set();

  appsMetadata.forEach(app => {
    if (!processedAppHostIds.has(app.appHostId)) {
      const cachedAppElement = cachedApps.apps[app.appHostId];
      // Managed Approuter needs a runtime url with sap.cloud.service, it may come from manifest.json or destination
      if (cachedAppElement && cachedAppElement.urls && cachedAppElement.urls.length > 0) {
        processedAppHostIds.add(app.appHostId);
        cachedAppElement.appFound = true;
        _addAppToResult(app, result.applications, cachedAppElement, runtimeHost);
      }
    }
  });
  ...
  for (let appHostId in cachedApps.apps) {
    if (!cachedApps.apps[appHostId].appFound) {
  ...
  for (let appHostId in cachedApps.apps) {
    const cachedAppElement = cachedApps.apps[appHostId];
    if (!cachedAppElement.appFound) {
      const destinationIdentifier = cachedAppElement.urls && cachedAppElement.urls.length > 0;
      if (destinationIdentifier) {
        result.errors.push(
          {
            destinationName: cachedAppElement.destinationName,
...
function _addAppToResult(app, result, cachedAppElement, runtimeHost) {
  // Managed Approuter needs a runtime url with sap.cloud.service, it may come from manifest.json or destination
  ...
  app.xsappname = cachedAppElement.xsappname;
  ...
  for (const url of cachedAppElement.urls) {
    const newApp = Object.assign({}, app);
    const appkey = dynamicRoutingUtils.getApplicationKey({ url });
    if (appkey) {
      newApp.applicationName = appkey.appNameWithoutPrefix;
      newApp.applicationVersion = appkey.appVersion;
      newApp.url = `${runtimeHost}${url.slice(1)}`;
      if (appkey.appDestinationId) {
        // instance destinations
        newApp.destinationId = appkey.appDestinationId;
      } else {
        if (embeddedCredsApp) {
          newApp.uniqueUrl = `${runtimeHost}${app.appHostId}.${sapCloudServiceNoDots}.${appkey.appNameWithoutPrefix}-${appkey.appVersion}`;
        }
      }
      result.push(newApp);
    }
...
function _addHTML5ApplicationsToCache(applicationsMetadata, req, cacheKey, addApps) {
  ...
  const returnLEPApplications = process.env.RETURN_LEP_APPLICATIONS === 'true';
  ...
  applicationsMetadata.forEach((app) => {
    ...
    const destinations  = handleDestinations(configuration);
    ...
    const skipApp       = (app.applicationType === technicalCacheBusterApplication && req.apiName) || (app.LEP && !returnLEPApplications);
...
async function _getHTML5RepoOptions(req, path, appHostId) {
  bsUtils = bsUtils || require('./business-service-utils');
  ...
  const requestOptions = {
    ...
    headers: {},
    enableRetry: true,
    timeout: vcapUtils.HTML5_APPS_REPO
  };
  if (appHostId){
  ...
  if (req.app.services && req.app.services[html5RepoService] && req.app.services[html5RepoService][req.tenant]
      ...
      && !tokenUtils.isTokenExpired(req.app.services[html5RepoService][req.tenant].token)) {
    ...
    const result = await tokenUtils.getClientCredentialsToken(req.tenant, headerUtils.getCorrelationId(req), tokenCredentials, null);
    if (result && result.errorMessage) {
      const errorMsg = 'Failed to get html5 apps repo token: ' + result.errorMessage;
      const error = new Error(errorMsg);
      if (tokenUtils.isInvalidSubaccount(result.errorMessage)) {
        error.status = 404;
      }
      throw error;
    }
    ...
    if (!req.app.services[html5RepoService][req.tenant]) {
      req.app.services[html5RepoService][req.tenant] = {};
    }
    req.app.services[html5RepoService][req.tenant].token = result;
    requestOptions.headers.Authorization = 'Bearer ' + req.app.services[html5RepoService][req.tenant].token.accessToken;
  }
  requestOptions.cache = {services: req.app.services, serviceName: html5RepoService, tenant: req.tenant};
...
function handleDestinations(configuration) {
  let destinations = configuration && configuration.destinations;
  ...
  destinations.forEach((destination) =>{
    // Store and temporarily remove IASDependencyName to avoid normalization
    const hasIASDependencyName = destination.hasOwnProperty('IASDependencyName');
    const originalIASDependencyName = destination.IASDependencyName;
    if (hasIASDependencyName) {
      delete destination.IASDependencyName;
    }

    let normalizedDestination = destinationNormalization.normalizeDestination(destination);

    // Restore original IASDependencyName after normalization
    if (hasIASDependencyName) {
      normalizedDestination.IASDependencyName = originalIASDependencyName;
      destination.IASDependencyName = originalIASDependencyName; // Restore to original object
    }
    normalizedDestination.forwardAuthToken = normalizedDestination.forwardAuthToken === 'true' || normalizedDestination.forwardAuthToken === true;
    if (normalizedDestination.timeout && !isNaN(normalizedDestination.timeout)) {
      normalizedDestination.timeout = _.toSafeInteger(normalizedDestination.timeout);
    }
    normalizedDestinations.push(normalizedDestination);
  });
  destinationNormalization.adjustDestinationProperties(normalizedDestinations);
  validators.validateDestinations(normalizedDestinations);
