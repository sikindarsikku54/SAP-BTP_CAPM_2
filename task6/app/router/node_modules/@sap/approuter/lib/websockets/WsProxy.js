...
function verifyClient(app, info, cb) {
  ...
  cacheServiceDestinations(req,{},function(err){
    ...
    handleExtension(req, {}, app.extensions, 'firstWS', (err) => {
      if (err) {
        return reportError(err, tracer, cb);
      }
      addRouterConfig(req, function (err) {
        ...
        attachZoneInfo(req, {}, function (err) {
          if (err) {
            return reportError(err, tracer, cb);
          }
...
function reportError(err, tracer, cb) {
  ...
  return cb(false, status, getErrorMessage(status, err.message, tracer, err.headers));
}
...
function createBackendConnection(req, cb) {
  ...
  handleTokenExchange(req,function (err) {
    ...
    const extensions = req.app && req.app.extensions ? req.app.extensions : [];
    handleExtension(req, {}, extensions , 'beforeRequestHandlerWS', (err) => {
      if (err) {
        return reportError(err, tracer, cb);
      }
      ...
      try {
        ...
        passportUtils.getAuthCertificates(req.session, targetUrl.destination, options);
...
function getErrorMessage(status, message, tracer, headers) {
  tracer.debug('[websockets] %s', message);
  if (headers) {
    tracer.debug('[websockets] headers: %o', headers);
  }
...
function getServiceToken(req){
  ...
  const serviceTag = req.destinationCredentials && req.destinationCredentials.uniqueServiceName && req.internalUrl.route.credentials && req.internalUrl.route.credentials.uniqueServiceName ? req.internalUrl.route.credentials.uniqueServiceName : req.internalUrl.route.service;
  let token = null;
...
function handleExtension(req, res, extensions, slot, cb) {
  if (process.env.HANDLE_WEBSOCKET_EXT && process.env.HANDLE_WEBSOCKET_EXT.toLowerCase() === 'true') {
    try {
      const tracer = getTracer(req);
      for (const extension of extensions) {
        const insert = extension.insertMiddleware;
        if (insert && insert[slot]) {
          const middlewareExtension = insert[slot];
          for (const m of middlewareExtension) {
            tracer.debug('Invoking extension: %s, path: %s', slot, m.path || '');
            if (typeof m === 'function') {
              m(req, res, cb);
            } else if (m.path) {
              if (isPathMatch(req, m.path.toLowerCase())) {
                m.handler(req, res, cb);
              }
            } else {
              m.handler(req, res, cb);
            }
  ...
  } else if (process.env.HANDLE_WEBSOCKET_EXT && process.env.HANDLE_WEBSOCKET_EXT.toLowerCase() === 'async') {
    handleExtensionAsync(req, res, extensions, slot, cb).catch(error => {
      const tracer = getTracer(req);
      tracer.error('Unhandled error in async WebSocket middleware:', error);
    });
  } else {
    cb();
  }
...
async function handleExtensionAsync(req, res, extensions, slot, cb) {
  const tracer = getTracer(req);
  try {
    for (const extension of extensions) {
      const insert = extension.insertMiddleware;
          ...
          if (handler) {
            await new Promise((resolve, reject) => {
              ...
              try {
                ...
                // Defensive fallback: if middleware returns a promise but forgets to call next()
                if (result && typeof result.then === 'function') {
                  result.then(() => {
                    if (!isResolved) {
                      // Fallback: middleware returned resolved promise but didn't call next()
                      isResolved = true;
                      resolve();
                    }
                  }).catch((error) => {
                    if (!isResolved) {
                      // Fallback: middleware returned rejected promise
                      isResolved = true;
                      reject(error);
                    }
                  });
                }
                // Primary path: wait for next() callback (AppRouter's standard expectation)
              } catch (error) {
                if (!isResolved) {
                  isResolved = true;
                  reject(error);
...
function isPathMatch(req, path) {
  let pathStart = 0, reqPath = req.url, reqPathLowerCase, isFQDN = req.url[0] !== '/';
  if (isFQDN) {
    const afterSchemaIdx = req.url.indexOf('://') + 3, queryStart = req.url.indexOf('?', afterSchemaIdx), slashStart = req.url.indexOf('/', afterSchemaIdx);
    pathStart = queryStart < 0 ? slashStart : Math.min(queryStart, slashStart);
    reqPath = reqPath.slice(pathStart);
  }
  reqPathLowerCase = reqPath.toLowerCase().split('?')[0];
  return path === '' || reqPathLowerCase.startsWith(path);
}
