...
const {generateScriptTagAndCspHeader} = require('../utils/csp-utils');
...
OAuth2Strategy.prototype.authenticate = async function (req, options) {
  ...
  if (!req.query || !req.query.code || (req.query.code && !loginCallbackProvider.isLoginCallback(req))) {
    ...
    if (process.env.PRESERVE_FRAGMENT !== 'false') {
      ...
      cookieUtils.signCookie(req, originalUrl, function (err, signature) {
        if (err) {
          returnResponseStatus(req, 500, 'Malformed server configuration');
          return;
        }
        const {openScriptTag, cspHeaderValue} = generateScriptTagAndCspHeader(logger);
  ...
  if (
    ...
    (await isStateProtectionEnabledForTargetApplication(req, cookies, logger))
  ) {
    const sessionCookieName = req.app && req.app.get('cookieName');
    const actualCookie = cookies[sessionCookieName];
    ...
    // Security fix: Look up expected cookie from state-to-cookie mapping
    // This prevents attackers who read the URL (and know state) from impersonating the session
    const expectedCookie = state.getStateToCookieMapping(req.query.state);
    if (!expectedCookie) {
      logger.error(`State parameter ${req.query.state} has no cookie mapping`);
      returnResponseStatus(req, 401, `State parameter ${req.query.state} is missing or invalid`);
      return;
    }
    if (actualCookie !== expectedCookie) {
      logger.error(`Session cookie does not match expected value for state ${req.query.state}`);
      returnResponseStatus(req, 401, 'Session cookie validation failed');
      return;
    }
  }
  const that = this;
  ...
  if (process.env.PRESERVE_FRAGMENT !== 'false') {
    ...
    if (!cookies) {
      logger.error('Cookies are missing for request url %s', getMaskedCallbackURL(req.url));
    ...
    cookieUtils.verifyCookie(req, redirectCookie, signatureCookie, function (err) {
      if (err) {
        logger.error('Signature does not match for request url %s', getMaskedCallbackURL(req.url));
...
function getAccessToken(req, that, callbackURL, loggingContext) {
  ...
  // PKCE: Retrieve code_verifier using SESSION COOKIE value (not state)
  const pkceEnabled = req.routerConfig && req.routerConfig.pkceEnabled;
  const stateParam = req.query.state;

  // Get the actual session cookie value for PKCE verifier lookup
  const cookies = cookie.parse(req.headers.cookie || '');
  const sessionCookieName = req.app && req.app.get('cookieName');
  ...
  function cleanupStateCache() {
    if (stateParam) {
      state.deleteCacheStateValue(stateParam);
      state.deleteStateToCookieMapping(stateParam);
      if (sessionCookie) {
        state.deleteCacheStateValue(sessionCookie);
      }
    }
    if (pkceEnabled && stateParam && sessionCookie) {
      state.deleteCodeVerifier(sessionCookie);
    }
    if (stateParam) {
      tracer.debug('State/PKCE cleanup completed for state: %s', stateParam);
    }
  }

  if (pkceEnabled && stateParam) {
    // Get verifier using session cookie value (secure method)
    const codeVerifier = sessionCookie ? state.getCodeVerifier(sessionCookie) : undefined;

    if (!codeVerifier) {
      // PKCE was used in authorization request, verifier MUST exist
      logger.error('PKCE code_verifier not found. Authorization session may have expired.');
      cleanupStateCache();
      const error = new Error('Authentication session expired. Please try login again.');
      error.status = 401;
      return callback(error);
    }

    tracer.debug('PKCE code_verifier retrieved using session cookie');
...
function getMaskedCallbackURL(url) {
  return url.replace(/code=([^&]*)/, (_, code) => {
...
/*
  * This function is used to check if state parameter is enabled for the target application,
 ...
 */
async function isStateProtectionEnabledForTargetApplication(req, cookies, logger) {
  const genErrMessage = 'Failed to evaluate state parameter enabled';

  // If html5-apps-repo is not used, stateProtection will be determined based on the central xs-app.json
  if (!drUtils.isDynamicRouting()) {
    return true;
  }
  ...
  // In order to calculate the getConfigurationsReq correct appKey, the dynamicRouting.appKey of the original request should be removed
  ...
  try {
    const appConfig = await drUtils.getConfigurations(getConfigurationsReq, appKey);
    return !(appConfig && appConfig.xsapp && appConfig.xsapp.stateProtection === false);
  } catch (e) {
