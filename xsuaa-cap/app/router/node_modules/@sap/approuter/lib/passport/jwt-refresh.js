...
module.exports = {
  ...
  refreshToken: function (app, session) {
    ...
    const memoryStore = app.get('memoryStore');
...
function doRefresh(app, session) {
  ...
  // Check if session still exists in store before attempting refresh
  const memoryStore = app.get('memoryStore');
  if (!memoryStore.exists(session.id)) {
    tracer.info('Session %s no longer exists, skipping JWT refresh', session.id);
    return;
  }
  ...
  passportUtils.callUaa(requestOptions, checkTimeout, function (err, uaaResponse) {
    ...
    let updatedSession;
    memoryStore.update(session.id, function updateSession(newSession) {
      if (!newSession) {
        return;
      }
      let options = {
