...
const {XSUAA} = require('../utils/vcap-utils');
...
module.exports = {
  ...
  getUserProperties: function (options) {
    ...
    return {
      ...
      isUserToken: Boolean(options.isUserToken || userName || userId),
      token: {
  ...
  getCallBackUrl: function (req) {
    ...
    const endpoint = req.routerConfig.appConfig.login.callbackEndpoint;
    const authType = pathUtil.pathAuthenticationType(req);
    let loginCallback;

    if (process.env.ENABLE_PERMANENT_AUTHTYPE === 'true' || multipleAuthTypes(req)) {
      loginCallback = endpoint + '?authType=' + authType;
    } else {
      loginCallback = endpoint;
    }
  ...
  callUaa: function (requestOptions, shouldDoRequest, cb) {
    ...
    function customRetryCondition(error) {
      if (error || error.response.statusCode !== 200) {
        const retryCount = error.config && error.config['axios-retry'] ? error.config['axios-retry'].retryCount : 0;
        tracer.info(error, 'Request attempt %d to authentication service at %s failed in %d ms.', retryCount + 1, requestOptions.url, Date.now() - requestStart);
        return true;
      }
      return false;
    }

    const requestOptionWithRetry = _.cloneDeep(requestOptions);
    requestOptionWithRetry.enableRetry = true;
    requestOptionWithRetry.service = XSUAA;
    requestOptionWithRetry.retryCondition = customRetryCondition;
    requestOptionWithRetry.retries = 2; // 2 retries, total 3 attempts

    request.post(requestOptionWithRetry, function onResponse(err, res, body) {
      ...
      try {
        ...
        tracer.info(err, 'Failed to parse response data');
