...
const hostPatternSchema = require('./schemas/refs/host-pattern-schema');
...
const circuitBreakerConfSchema = require('./schemas/circuit-breaker-conf-schema.json');
let regexErr;
...
module.exports = {
  ...
  validateCircuitBreakerConf: function (circuitBreakerConf) {
    const validator = new JsonValidator();
    validator.validate(circuitBreakerConf, circuitBreakerConfSchema, 'CIRCUIT_BREAKER_CONF');
  },
  ...
  validateXsApp: function (configuration, envDestinations, directory) {
    ...
    validator.addFormat('route-rules', function (route) {
      ...
      if (route.identityProvider && route.authenticationType && (route.authenticationType !== 'xsuaa' && route.authenticationType !== 'ias')) {
        return 'Route has both identityProvider and authenticationType that is not of type \'xsuaa\' or \'ias\'';
      }
      if (route.localDir) {
    ...
    validator.addFormat('logout-rules', function (logout) {
      ...
      if (logout.backChannelLogoutEndpoint && logout.logoutEndpoint && (logout.logoutEndpoint === logout.backChannelLogoutEndpoint)) {
        return 'Logout endpoint must be different from backChannelLogoutEndpoint';
      }
    });
  ...
  validateCors: function (options) {
    ...
    validator.addSchema('hostPatternSchema', hostPatternSchema);
    validator.addSchema('sourceSchema', sourceSchema);
