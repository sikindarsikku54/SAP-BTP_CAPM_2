...
exports.getHeaders = function (req, accessToken, destination) {
  ...
  if (req) {
    ...
    const destinationId = req.destinationKey || (req.destinationCredentials && req.destinationCredentials.destination && req.destinationCredentials.destination.instanceid);
    const destinationKey = destinationId && destination && destination.name ? destinationId + '-' + destination.name : (destination && destination.name);
...
function addOauthHeader(headers, tokens, route, destination, logger) {
  ...
  if ((route && route.destination) || (destination && destination.isLogout)) {
    ...
    if (businessServiceUtils.getGrantType(route.credentials) === 'client_credentials') {// html5 apps repo flow
      if (!tokens[route.service]) {
        throw new Error(`No token found for service ${route.service}`);
      }
