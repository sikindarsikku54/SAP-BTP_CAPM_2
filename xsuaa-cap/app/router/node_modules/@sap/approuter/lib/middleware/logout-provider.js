...
module.exports = {
  ...
  callServiceLogoutPaths: function (req, session) {
    ...
    for (let serviceTag in logoutServices) {
      ...
      if (serviceLogoutOptions && serviceLogoutOptions.logoutPath) {
        const serviceCredentials = businessServiceUtils.getCredentials(serviceTag, false, req || {session: session});
        ...
        serviceTokenHandler.replaceUserToken(req, session, serviceTag, false)
          .then(token => {
            ...
            logger.error(err, 'Error replacing user token for service ', serviceTag);
          });
      }
    }
  },
...
function resolveLogoutRedirectUrl(req, cb) {
  ...
  passportUtils.getAuthConfig(req, req.authenticationType, function (err, authConfig) {
    ...
    if (process.env.LOGOUT_WITH_TOKEN_HINT && process.env.LOGOUT_WITH_TOKEN_HINT.toLowerCase() === 'true') {
      additionalQueryParams = isIas && req.idToken ? '&id_token_hint=' + req.idToken : '&client_id=' + authConfig.clientid;
    }
    else {
      const apptid = req.zoneInfo && req.zoneInfo.apptid;
      additionalQueryParams = isIas && apptid ? '&app_tid=' + apptid +  '&client_id=' + authConfig.clientid : '&client_id=' + authConfig.clientid;
    }
