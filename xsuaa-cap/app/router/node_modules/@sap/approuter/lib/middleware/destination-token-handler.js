...
const destinationNormalization = require('../utils/destination-normalization');
...
const self = module.exports = {
  ...
  retrieveDestination: function (options, cb) {
    self.replaceUserToken(options, function (err, userExchangeToken, token, iasTokenFlow) {
      if (err) {
        return cb(err);
      }
      options.useIASMTLS = iasTokenFlow;
      ...
      destinationUtils.findDestination(destinationName, accessToken, options, function (err, destinationLookUpResult) {
        ...
        try {
          ...
          destinationNormalization.normalizeDestinationProperties(destinationsConfigurations);
          ...
          destinationNormalization.adjustDestinationProperties(destinationsConfigurations);
          ...
          result.destination.iasTokenFlow = iasTokenFlow;
  ...
  replaceUserToken: function (options, cb) {
    ...
    if (!session || !session.user || !session.user.isUserToken) {
      return cb(null);
    }
    shouldRequestUserExchangeToken(session, options.destinationKey, function (err, askForToken) {
      ...
      if (!askForToken) {
        ...
        // Forwarding IAS token to destination service is only support for subaccount level destinations
        const externalServiceCredentials = options.destinationCredentials || destinationUtils.getDestinationServiceCredentials();
        if (!options.destinationKey && destinationUtils.useIASToken(options, externalServiceCredentials)) {
          let jwt = options.jwt || options.session.user.token.accessToken;
          return cb(null, jwt, null, true);
        }
      ...
      sessionExt.update(options.session, function(session) {
        ...
        session.user.iasDependencies[IASDependencyName] = {
          token: {
            ...
            expiresAt: Math.floor(Date.now() / 1000) + token.expires_in - 20
          }
        };
      });
...
function getExternalServiceCredentials(options, cb){
  ...
  if (options.destinationKey && options.app && options.app.services[options.destinationKey]) {
    destinationUtils.getDestinationCredentialsByDestinationKey(options.app, options.destinationKey, options.correlationId, (err, destCredentials) => {
      ...
      externalServiceCredentials.uaadomain = options.uaadomain;
