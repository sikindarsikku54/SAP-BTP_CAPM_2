...
const {DESTINATION} = require('../utils/vcap-utils');
...
exports.getVcapServicesData = getVcapServicesData;
exports.getConfigurations = promisify(getConfigurations);
...
function getRemoteConfiguration(req, res, next) {
  const logger  = req && req.loggingContext && req.loggingContext.getLogger('/get remote configuration');
  const useLocationAfterLogin = process.env.USE_HTML5_APP_XS_APPJSON_IN_LOGIN_CALLBACK === 'true';
  useLocationAfterLogin && logger && logger.info('Using html5 application xs-app.json in login callback');
  let appKey = getApplicationKey(urlUtils.getFirstUrlSegment(req,useLocationAfterLogin));
  if (!appKey) {
    return next();
  }
...
function getConfigurations(req, appKey, callback) {
  const logger  = req && req.loggingContext && req.loggingContext.getLogger('/get remote configuration');
  const cacheKey = req && req.tenant && !appKey.appDestinationId ? `${req.tenant}_${appKey.entireKey}` : appKey.entireKey;
  ...
  getVcapServicesData(req, appKey, function (err, vcapServicesData) {
    ...
    request.get(vcapServicesData.requestOptions, function onResponse(err, res, body) {
      if (err || !res || !res.statusCode) {
        ...
        logger && logger.debug && logger.debug(`Got ${HTML5_APP_NOT_FOUND} response status while retrieving application ${appKey.applicationName}, using central approuter xs-app.json.`);
        tracer.info('Application %s does not exist', cacheKey);
        configCache.set(cacheKey, HTML5_APP_NOT_FOUND);
        if (process.env.SAAS_APPROUTER) {
      ...
      let success = configCache.set(cacheKey, config);
      if (!success) {
...
function getVcapServicesData(req, appKey, cb) {
  ...
  getHTML5RepoToken(req, html5RepoCredentials, function (err, html5RepoToken) {
    const logger  = req && req.loggingContext && req.loggingContext.getLogger('/Dynamic routing');
    if (err) {
      return cb(err);
    }
    const ownSapCloudService = isOwnSapCloudService(appKey.appPrefix, req);
    if (appKey && appKey.appPrefix && !ownSapCloudService) {
      ...
      if (!bsCredentials || !bsCredentials[HTML5_APPS_REPO_NAME]) {
        return cb('Invalid VCAP credentials for business service: html5-apps-repo part is missing');
      }
      if (!bsCredentials[HTML5_APPS_REPO_NAME]['app_host_id']) {
    ...
    let requestOptions = {};
    const tenantInfo = req && passportUtils.getTenantInfo(req);
    ...
    requestOptions.cache = {services: req.app.services, serviceName: html5RepoService, tenant: (tenantInfo && tenantInfo.tenant)};
    requestOptions.enableRetry = true;
    requestOptions.service = DESTINATION;

    const logInfo = tenantInfo && tenantInfo.logInfo;
    logInfo && (requestOptions.headers.tenantid = logInfo);

    if (bsCredentials) {
      if (bsCredentials[HTML5_APPS_REPO_NAME] && bsCredentials[HTML5_APPS_REPO_NAME]['app_host_id']) {
        requestOptions.headers['x-app-host-id'] = bsCredentials[HTML5_APPS_REPO_NAME]['app_host_id'];
      }
...
function getApplicationKey(req,segmentIndex) {
  ...
  } else if (parts.length === 3 && (req.isOwnSapCloudService || isOwnSapCloudService(parts[1], req))) {
    appKey.appPrefix =  parts[1];
    appKey.appNameWithoutPrefix = parts[2];
  } else {
  ...
  if (appKey && appNameRegex.test(appKey.appNameWithoutPrefix)){
    if (process.env.SAAS_APPROUTER && !segmentIndex){
      ...
      html5RepoUtils.attachCredentials(appKey, (bsKey && bsKey.appPrefix),contextId, req);
      appKey.bsKey = bsKey && bsKey.appPrefix && html5RepoUtils.getCredentials(bsKey.appPrefix,req) ? bsKey : undefined;
    } else if (!process.env.SAAS_APPROUTER && appKey.appPrefix && isOwnSapCloudService(appKey.appPrefix, req) && !segmentIndex) {
      req.isOwnSapCloudService = true;
      const bsKey = pathSegments[2] && exports.getApplicationKey(req,2);
      if (bsKey && businessServiceUtils.getCredentials(bsKey.appPrefix, true, req)) {
        appKey.bsKey = bsKey;
      }
    }
    return appKey;
...
function fetchProviderSubaccountToken(req, html5RepoCredentials, cb) {
  if (!req.app.services || !req.app.services[html5RepoService]
      || !req.app.services[html5RepoService].token || tokenUtils.isTokenExpired(req.app.services[html5RepoService].token)) {
...
function fetchSubscriberSubaccountToken(req, tenant, html5RepoCredentials, cb) {
  if (req.app.services && req.app.services[html5RepoService] && req.app.services[html5RepoService][tenant]
      ...
      && !tokenUtils.isTokenExpired(req.app.services[html5RepoService][tenant].token)) {
    ...
    tokenUtils.getClientCredentialsTokenByTenant(headerUtils.getCorrelationId(req), tenant, html5RepoCredentials, req.zoneInfo, (err, token) => {
      if (err) {
        if (tokenUtils.isInvalidSubaccount(err)) {
      ...
      if (!req.app.services[html5RepoService][tenant]) {
        req.app.services[html5RepoService][tenant] = {};
      }
