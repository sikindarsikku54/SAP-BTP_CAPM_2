...
module.exports.logRequestSetTenant = function(req, tenant) {
  if (req && req.logger) {
    ...
    let dynamicLogLevel;
    const acceptExternalLogLevel = process.env.ACCEPT_EXTERNAL_LOG_LEVEL && process.env.ACCEPT_EXTERNAL_LOG_LEVEL.toLowerCase() === 'true';
    dynamicLogLevel = acceptExternalLogLevel && req.headers && (req.headers['X-Log-Level'] || req.headers['x-log-level']);
    dynamicLogLevel && (delete req.headers['X-Log-Level']) && (delete req.headers['x-log-level']);
    !dynamicLogLevel && (dynamicLogLevel = module.exports.getDynamicLogLevel(req));
    if (dynamicLogLevel){
  ...
  request.get(requestOptions, function(err, response, body) {
    if (err || response.statusCode !== 200) {
      const error = new Error('Failed to execute query ' + requestOptions.body + ' response status ' +
        (response ? response.statusCode : '') + ' ' + (body ? body : '') + err);
      return cb(error);
    }
