...
const {XSUAA} = require('./vcap-utils');
...
function addSubscribedApplications(req, html5AppsResponse, cb) {
  ...
  let html5RepoCredentials = vcapUtils.getServiceCredentials({tag: html5RTService});
  html5RepoCredentials.label = html5RTService;
  tokenUtils.getClientCredentialsTokenByTenant(headerUtils.getCorrelationId(req), tenant, html5RepoCredentials, null, (err, token) => {
    ...
    exports.fetchSubscribedApplications(tenant, token, html5RepoCredentials, tracer, req.app.services, (err, subscribedApplications) => {
...
function fetchSubscribedApplications(tenant, token, html5RepoCredentials, tracer, services, cb) {
  let requestOptions = {
    url: html5RepoCredentials.uri + '/applications/subscriptions',
    headers: {
      Authorization: 'Bearer ' + token.accessToken
    },
    cache: {services: services, serviceName: html5RTService},
    enableRetry: true,
    service: XSUAA
  };
