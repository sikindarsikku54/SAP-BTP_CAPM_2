...
const stdTTL = getValidatedEnvInt('SERVICE_DESTINATION_CACHE_TTL', 300);
const serviceInstanceDestinationsCache = new NodeCache({stdTTL: stdTTL, checkperiod: 16});
const DESTINATION = vcapUtils.DESTINATION;
...
function getServiceInstanceDestinationsByTenant(req, cb) {
  ...
  getDestinationInstanceCredentialsByTenant(req, function (err, tenantDestinations) {
    ...
    const chunkSize = getValidatedEnvInt('FETCH_DESTINATIONS_CHUNK_SIZE', 50);
    const delay = getValidatedEnvInt('FETCH_DESTINATIONS_CHUNK_DELAY', 1000);
...
async function getDestinationsByDestinationKey(app, destinationKey,destinationConfiguration, correlationId, runtimeFlow, cb) {
  ...
  if (!destinationToken || destinationToken.tokenRefreshTimestamp - Date.now() <= 0 || !app.services[destinationKey].credentials) {
    if (destinationConfiguration) {
      ...
      const _destinationId = destinationKey.split('_')[1];
      ...
      try {
        ...
        logger.error(`No destination found for destination key ${destinationKey}, fallback to destinationId ${_destinationId}, error: ${err}`);
        try {
          response = await findDestinationPromise(_destinationId, null, {app: app, correlationId: correlationId});
        } catch (err) {
...
function getFindDestinationReqOptions(destinationName, token, options, cb) {
  ...
  getDestinationToken(options, token, credentials, (err, options) => {
    ...
    shouldCache && (requestOptions.cache = {services: options.app.services, serviceName: options.destinationKey || DESTINATION, tenant: options.urlTenant});
...
async function executeInChunks(promises, chunkSize, delay) {
  ...
  for (let i = 0; i < promises.length; i += chunkSize) {
    ...
    const chunkResults = await Promise.all(chunk).catch((err) => {
      logger.error(`Could not fetch destinations chunk, error ${err}, continue with next chunk`);
    });
    // in case of error chunkResults will be undefined
  ...
  // the worst case it's empty not undefined
  return results;
}

function useIASToken(options, credentials) {
  const token = options.session && options.session.user && options.session.user.token;
  const tenant = options.tenant || (options.session && options.session.user && options.session.user.tenant);
  let enableForwardCorporateIDPToken = false;
  if (process.env.ENABLE_FORWARD_CORPORATE_IDP_TOKEN && process.env.ENABLE_FORWARD_CORPORATE_IDP_TOKEN.toLowerCase() === 'true') {
    enableForwardCorporateIDPToken = true;
  } else if (process.env.ENABLE_FORWARD_CORPORATE_IDP_TOKEN && process.env.ENABLE_FORWARD_CORPORATE_IDP_TOKEN.toLowerCase() !== 'false') {
    const tenantList = new Set(process.env.ENABLE_FORWARD_CORPORATE_IDP_TOKEN.split(',').map(tenant => tenant.toLowerCase().trim()));
    enableForwardCorporateIDPToken = tenantList.has(tenant.toLowerCase());
  }
  ...
  logger.info(`useIASToken - token forwardCorpIdpToken: ${!!(token && token.forwardCorpIdpToken)}, authenticationTypeIAS: ${authenticationTypeIAS}, iasTokensSupported: ${iasTokensSupported}, enableForwardCorporateIDPToken: ${enableForwardCorporateIDPToken}`);
  return !!(((token && authenticationTypeIAS && token.forwardCorpIdpToken) || (options.clientCredentialsFlow && options.iasSubscription)) && iasTokensSupported && enableForwardCorporateIDPToken);
}
