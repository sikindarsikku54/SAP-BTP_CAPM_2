...
- [*xs-app.json* configuration file](#xs-appjson-configuration-file)
  ...
  * [*outgoingRequestOptions* property](#outgoingrequestoptions-property)
...
| PKCE                                                                           | `PKCE_ENABLED`                                  | Enables Proof Key for Code Exchange (PKCE) for the OAuth2 authorization code flow. PKCE is enabled by default when `STATE_PARAMETER_SECRET` is set. Disabling state protection also disables PKCE. Set to `false` to disable PKCE while keeping state protection enabled.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Extended Content-Security-Policy Header for Login                              | EXTENDED_LOGIN_CSP                              | By default, the application router applies basic Content-Security-Policy (CSP) headers during login. To enable extended CSP headers that include the additional directives `default-src`, `style-src`, `img-src` (including `data:`), `connect-src`, `font-src`, `base-uri`, and `form-action`, set this environment variable to `true`. This configuration enhances the security during authentication by explicitly defining allowed sources for default content, styles, images, data, connection, font, base URI and form submissions. This environment variable is disabled by default to maintain backward compatibility.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Omit Content-Security-Policy Header for Login                                  | OMIT_LOGIN_CSP                                  | When set to `true`, the application router will not send any Content-Security-Policy (CSP) header during the login flow. This allows approuter owners to completely disable CSP headers in the login response if needed. This environment variable is disabled by default.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| Cookie backward compatibility                                                  | COOKIE_BACKWARD_COMPATIBILITY                   | If 'true', cookie serialization will preserve previous cookie library version serialization algorithm.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Error source tracking                                                          | `RETURN_ERROR_SOURCE`                           | When set to `true`, the application router adds an `x-error-source` header to error responses only for errors that originate within the application router itself. The header value will always be `gateway`. Errors from remote integrations will not include this header. Default: disabled (no header added).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               
| Zone information cache time to live                                            | `ZONE_INFO_CACHE_TTL`                           | Time in seconds for zone information to be cached. The default value is 600 seconds.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           
| Zone information maximum cache TTL                                             | `ZONE_INFO_MAX_CACHE_TTL`                       | Maximum time in seconds for unused zone information entries to stay in cache before automatic cleanup. The default value is 14400 seconds (4 hours).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| Disable HTTP connection reuse                                                  | `DISABLE_CONNECTION_REUSE`                      | Controls HTTP connection reuse behavior for **outgoing connections** from the application router to backend destinations and business services. This setting does not affect incoming connections from browsers or external proxies to the application router. Set to `true` to disable connection reuse (each request uses a new connection). Set to `false` to enable connection reuse (keep-alive connections). When not set, the default behavior depends on the environment: in Cloud Foundry, connection reuse is **enabled** by default. In Kyma/Kubernetes and SAP Business Application Studio (BAS), connection reuse is **disabled** by default.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Enable WebSocket Extensions                                                    | `HANDLE_WEBSOCKET_EXT`                          | Enables WebSocket extensions for custom middleware. Possible values: `false` (default) - WebSocket extensions disabled; `true` - synchronous WebSocket extensions enabled; `async` - async WebSocket extensions with proper await handling. See [WebSocket Extensions](doc/extending.md#websocket-extensions).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
[Outgoing request options](#outgoingrequestoptions-property) | `OUTGOING_REQUEST_OPTIONS`                      | Configuration regarding outgoing request options to services like destination service etc.
...
Here are the Approuter limitations to destination properties configuration :
...
HTML5.StrictConnectionMode |          x          | If `true`, the application router uses strict HTTP agents with conservative connection pooling settings (shorter socket timeouts, FIFO scheduling) for this destination. This is recommended for mTLS/certificate-based authentication scenarios. Default value is `false`.
...
authenticationType | String | x | Specifies the authentication method for the route.</br></br>**Values:**</br>• `ias` - Identity Authentication service (redirects to IAS login)</br>• `xsuaa` - User Account and Authentication service (redirects to XSUAA login)</br>• `basic` - HTTP Basic authentication (supports HANA users, SAP ID service, IAS)</br>• `none` - No authentication required</br></br>**If `authenticationType` is not defined:**</br>• Bound to IAS only: uses `ias`</br>• Bound to XSUAA only: uses `xsuaa`</br>• Bound to both services: prefers ias if both authentication services can be used in the current subaccount, determined via BTP security endpoint (may impact response time)</br></br>**Note:** For Basic authentication options, see SAP Note 3015211.
...
If dynamicIdentityProvider is set to true, the end user can set the identity provider (IDP) for the application’s login process by filling the request query parameter `sap_idp`. The value is passed as the `idp` parameter to the authentication service (XSUAA or IAS).

When using SAP Authorization and Trust Management (XSUAA) with Identity Authentication Service (IAS), you can specify a chain of identity providers using comma-separated values. For example, `sap_idp=sap.custom,local` will route through the XSUAA trust configuration `sap.custom` and then to IAS with `idp=local` to use the IAS local user store.

For more information on identity provider selection, see [Creating URLs for Applications to Authenticate with Specific Identity Providers](https://help.sap.com/docs/cloud-identity-services/cloud-identity-services/creating-urls-for-applications-to-authenticate-with-specific-identity-providers-44e67223a98f45ed94aaee7513e90a95).
...
setBackendSessionCookies | Boolean | x | If this property is set to true, the application router returns session cookies from the backend applications (cookies without an expiration date) to the browser instead of storing them only in the user session. You usually need this setting when the application router does not manage the user sessions and the `authenticationType` property is set to `none`.
...
When state protection is enabled, PKCE (Proof Key for Code Exchange) is also applied by default to the OAuth2 authorization code flow. PKCE can be disabled independently via the `PKCE_ENABLED` environment variable.
...
### *outgoingRequestOptions* property
Outgoing requests divided into services. By default lib/configuration/outgoing-request-options.js used.

Here is the contents of the default outgoing-request-options.js file in json format:
```json
{
  "cls": {
    "timeout": 0
...
Property | Type    | Optional | Description
-------- |---------|:--------:| -----------
timeout | Integer | x | Outgoing request timeout in milliseconds. Zero (0) means no timeout. Exceeding > 0 timeout results in breaking connection.


Example of globally configuring timeout for xsuaa service using the environment variable `OUTGOING_REQUEST_OPTIONS`:
```json
{
  "xsuaa": {
    "timeout": 20000
...
Here is an example of the outgoing request options environment variable:
```json
  "env":
   OUTGOING_REQUEST_OPTIONS: >
  {
   "xsuaa": {
    "timeout": 20000
  }
...
With the outgoingRequestOptions property, you can configure options for outgoing requests.<br> 
For more information about the outgoingRequestOptions configuration see [Outgoing Request Options](#outgoing-request-options-configuration).
...
**Important Note for Managed Approuter with IAS Multitenancy**:
When using the SAP Managed Approuter (Launchpad/Workzone service) with IAS authentication in a multitenant scenario, the IAS provider/tenant must have multitenancy support enabled. If the IAS provider does not support multitenancy, the managed approuter will not be able to subscribe to the IAS tenant, and subscription attempts will fail. This prevents tenant onboarding for multitenant applications.

To resolve this:
- Ensure the IAS tenant is configured to support multitenancy in the IAS admin console
- Or use a different IAS tenant that has multitenancy support enabled
- Or use XSUAA instead of IAS if multitenancy is required and IAS multitenancy cannot be enabled

This requirement only applies to multitenant applications using managed approuter with IAS authentication. Single-tenant applications are not affected.
...
* **rejectUnauthorized** - a flag that indicates whether the Redis certificate should be rejected if it is not authorized by a trusted CA. The default value is true.
...
```json
{
    ...
    "skipCertHostnameValidation": false,
...
> **_NOTE:_** CCEE provided Redis currently only supports self-signed certificates. Set `rejectUnauthorized` to `false` and `skipCertHostnameValidation` to `true`.
