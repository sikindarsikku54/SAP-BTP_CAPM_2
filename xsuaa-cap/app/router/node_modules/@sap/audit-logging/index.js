...
function _getServiceOptions(credentials) {
  ...
  const cacheKey = `${AUDIT_V2_OPTIONS_CACHE_KEY}-${uri}`;
  if (auditCache.has(cacheKey)) {
    return Promise.resolve(auditCache.get(cacheKey));
  }
  ...
  return fetch(uri, options).then(function (res) {
    ...
    auditCache.set(cacheKey, v2ServiceOptions);
